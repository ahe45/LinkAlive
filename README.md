...
웹 origin으로 사용합니다. 브라우저의 API 요청은 웹 서버가 내부 API로 전달하므로 외부 장치는
3001번 포트만 사용합니다. 특정 호스트명이나 주소를 사용하려면 실행 전에 `LINKALIVE_HOST`를
설정하세요. 다른 장치에서 접속하려면 Windows 방화벽에서 TCP 3001 포트를 로컬 서브넷에
허용해야 합니다.
...
새 모니터에는 등록한 계정이 소유자로 저장되며, 해당 소유자와 관리자만 모니터를 수정하거나
삭제하고 일시 중지·재개할 수 있습니다.
